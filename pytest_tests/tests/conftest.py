...
def test_platform(server):
    ...
    test_platform = TestPlatform(ip, rest_port=int(port), platform=platform)
...
def ixnetwork(test_platform):
    ...
    server_obj = Server(
        test_platform._connection._hostname,
        test_platform._connection._rest_port,
        test_platform._connection._platform
    )

    # getting sessions as per test platform
    if server_obj.platform == 'windows':
        session = test_platform.Sessions.find(Id=1)
    elif server_obj.platform == 'linux':
        test_platform.Authenticate('admin', 'admin')
        session = test_platform.Sessions.add()
    elif server_obj.platform == 'connection_manager':
        session = test_platform.Sessions.add()
    # getting ixnetwork objects from session and returning it
